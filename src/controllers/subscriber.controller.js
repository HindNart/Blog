...
module.exports = {
    ...
    unsubscribe: catchAsync(async (req, res) => {
        ...
        if (!token) return res.render('unsubscribe', {
            title: 'Hủy đăng ký',
            error: 'Token không hợp lệ'
        });

        const sub = await Subscriber.findOne({ unsubscribeToken: token });
        if (!sub) return res.render('unsubscribe', {
            title: 'Hủy đăng ký',
            error: 'Token không hợp lệ hoặc đã hủy'
        });
        ...
        res.render('unsubscribe', {
            title: 'Hủy đăng ký',
            success: 'Bạn đã hủy đăng ký nhận tin thành công.'
        });
    }),
};